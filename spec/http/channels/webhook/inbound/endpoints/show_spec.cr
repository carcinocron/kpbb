...
describe "Http::Channels::Webhook::Inbound::Endpoints::Show" do
  it "rejects user without membership" do
    ...
    req = HTTP::Request.new("GET", "#{channel.relative_url}/webhook/inbound/endpoints/#{endpoint.id.to_b62}", default_browser_get_headers.merge!(accepts_json), "")
    res = process_request req
    res.should be_json_401

    req = HTTP::Request.new("GET", "#{channel.relative_url}/webhook/inbound/endpoints/#{endpoint.id.to_b62}", default_browser_get_headers, "")
    res = process_request req
    res.should be_redirect_login

    req = user.request("GET", "#{channel.relative_url}/webhook/inbound/endpoints/#{endpoint.id.to_b62}", default_browser_get_headers.merge!(accepts_json), "")
    res = process_request req
    res.should be_json_403

    req = user.request("GET", "#{channel.relative_url}/webhook/inbound/endpoints/#{endpoint.id.to_b62}", default_browser_get_headers, "")
    res = process_request req
