...
module Iom::Spec
  # struct JustIseCodeFromJson
  #   include JSON::Serializable

  #   property code : ::JSON::Any
  # end

  struct BeHtmlRedirectResponseExpectation
    def initialize(@expected_header_location : String)
      @expected_status_code = 302
      @expected_body = ""
    end

    def match(actual_res)
      # body = JustIseCodeFromJson.from_json(actual_res.body)

      # @expected_body = body.to_json

      if actual_res.headers["Location"]?.try(&.starts_with?(@expected_header_location))
        @expected_header_location = actual_res.headers["Location"]? || @expected_header_location
      end

      expected_value = <<-RAW
      #{@expected_status_code}
      Accept: text/html; charset=utf-8
      Location: #{@expected_header_location}
      #{@expected_body}
      RAW
      actual_value = <<-RAW
      #{actual_res.status_code}
      Accept: #{actual_res.headers["Content-Type"]?}
      Location: #{actual_res.headers["Location"]?}
      #{actual_res.body}
      RAW
