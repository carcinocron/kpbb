...
# Build App
RUN shards build server --error-trace --production

# Extract dependencies
RUN ldd bin/server | tr -s '[:blank:]' '\n' | grep '^/' | \
...
COPY --from=0 /src/bin/server /server
...
ENV PG_URL=postgresql://kpbb:password@127.0.0.1:5432/kpbb?sslmode=require
...
# ENV FN_INBOUND_FEED https://us-west1-REDACTED_GOOGLE_CLOUD_PROJECT_ID.cloudfunctions.net/inbound_feed

ENV YOUTUBE_API_KEY=REDACTED_YTKEY
ENV RAVEN_OS_CONTEXT_EXEC=alpine-kpbb-server

# Run the app binding on port 8080
EXPOSE 8080
ENTRYPOINT ["/server"]
HEALTHCHECK CMD ["/server", "-c", "http://127.0.0.1:8080/"]
CMD ["/server", "-b", "0.0.0.0", "-p", "8080"]
