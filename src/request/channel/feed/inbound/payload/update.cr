...
module Kpbb::Request::Channel::Feed::Inbound::Payload
  struct Update
    property model : Kpbb::Feed::Inbound::Payload
    ...
    def initialize(@model : Kpbb::Feed::Inbound::Payload, @channel : Kpbb::Channel, @env : HTTP::Server::Context)
      ...
      @query = "UPDATE feed_inbound_payloads SET "
      ...
      if @input.has_key? "reset_result"
        if !@model.result.nil? && @input.truthy?("reset_result")
          @channellogs << Kpbb::DraftChannelLog.new(action: Kpbb::ChannelAction::UpdateFeedInboundPayloadResetResult)
          @model.result = nil
          @sets << "result = NULL"
        end
      end
      ...
      #     @channellogs << Kpbb::DraftChannelLog.new(action: Kpbb::ChannelAction::UpdateFeed::InboundPayloadActive)
      #     @model.active = false
    ...
    # include MoreAccord
    # validates_with [
    #   Kpbb::Validator::Channel::Feed::Inbound::Payload::Bio,
    # ]
