module Kpbb::Feed::Inbound
  ...
  struct Payload
    ...
    @@table = "feed_inbound_payloads"

    def self.select_columns : Array(String)
      ["id", "channel_id", "endpoint_id", "guid", "path",
    ...
    def self.save!(
      ...
      query = <<-SQL
        ...
        VALUES (
          (SELECT channel_id FROM feed_inbound_endpoints
            WHERE id = $1),
          $2, $3, $4,
          $5, $6,
          NOW(), NOW())
      ...
      self.new(id, channel_id, endpoint_id, guid, path,
        data, result, created_at, updated_at)
    end
    ...
    def self.fetch_page(env) : Page(self)
      ...
      if channel_id
        where << "feed_inbound_payloads.channel_id = #{nqm.next}"
        bindings << channel_id
      end
      endpoint_id = (env.params.url["endpoint_id"]? || env.params.query["endpoint_id"]?).try(&.to_i64_from_b62?)
      if endpoint_id
        where << "feed_inbound_payloads.endpoint_id = #{nqm.next}"
        bindings << endpoint_id
      end
      guid = (env.params.url["guid"]? || env.params.query["guid"]?)
      if guid
        where << "feed_inbound_payloads.guid = #{nqm.next}"
        bindings << guid
      end
      ...
      pagination_before_after "feed_inbound_payloads"
      if where.size > 0
    ...
    def self.fetch_one_per_endpoint(minute : Time) : Array(self)
      ...
      query = <<-SQL
        ...
        FROM (
          ...
          JOIN feed_inbound_endpoints as endpoints ON endpoints.id = payloads.endpoint_id
          WHERE payloads.result IS NULL
          AND payloads.data IS NOT NULL
