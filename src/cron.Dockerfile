...
# Build App
RUN shards build cron --error-trace --production

# Extract dependencies
RUN ldd bin/cron | tr -s '[:blank:]' '\n' | grep '^/' | \
...
ENV RAVEN_OS_CONTEXT_EXEC=alpine-kpbb-cron

# Run the app binding on port 8080
EXPOSE 8080
ENTRYPOINT ["/cron"]
# HEALTHCHECK CMD ["/cron", "-c", "http://127.0.0.1:8080/"]
CMD ["/cron"]
